...
function CourseCard({ course }: { course: AICourse }) {
  ...
  return (
    <div
      ...
      className="course-card relative group transition-all duration-300 ease-out bg-white dark:bg-[#13111C] rounded-3xl border border-[#EAE6FE] dark:border-white/10 overflow-hidden flex flex-col h-full shadow-sm hover:border-[#8B7FE8] dark:hover:border-[#8B7FE8]/60"
      style={{
      ...
      <div className="p-5 sm:p-6 flex-1 flex flex-col justify-between">
        <div>
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-lg sm:text-xl font-extrabold text-[#1E1B2E] dark:text-white tracking-tight group-hover:text-[#8B7FE8] transition-colors">
              {course.title}
            </h3>
            <div className="flex items-center gap-1 bg-[#F3F0FE] dark:bg-[#1E1933] border border-[#EAE6FE] dark:border-white/10 px-2 py-0.5 rounded-full text-[11px] font-bold text-[#1E1B2E] dark:text-white">
              <Star className="w-3 h-3 fill-[#8B7FE8] text-[#8B7FE8]" />
              <span>{course.rating}</span>
            </div>
          </div>
          
          <p className="text-xs text-[#6B6785] dark:text-[#A09CAE] leading-relaxed mb-4 line-clamp-2">
            {course.description}
          </p>
          ...
          <div className="flex flex-wrap gap-1.5 mb-4">
            {course.tags.slice(0, 3).map((tag, i) => (
              <span
                key={i}
                className="px-2.5 py-0.5 rounded-md text-[10px] font-semibold bg-[#F3F0FE] dark:bg-[#1A1827] text-[#8B7FE8] dark:text-[#D8D2FA] border border-[#EAE6FE] dark:border-white/5"
              >
                {tag}
              </span>
            ))}
        ...
        <div className="pt-4 border-t border-[#EAE6FE] dark:border-white/10 flex items-center justify-between mt-auto">
          <span className="text-[10px] font-bold text-[#6B6785] dark:text-[#8E8A9F] uppercase tracking-wider flex items-center gap-1">
            <BookOpen className="w-3 h-3 text-[#8B7FE8]" />
            Interactive
          </span>
          <Link
            href={`/courses/${course.id.replace("course-", "")}`}
            className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-bold text-white bg-[#8B7FE8] hover:bg-[#786BD6] shadow-soft-sm transition-all active:scale-95 group/btn min-h-[38px]"
          >
            <span>Start</span>
...
export default function DashboardCourseGrid() {
  ...
  return (
    <div className="w-full">
      ...
      <div className="bg-white dark:bg-[#13111C] rounded-3xl p-4 sm:p-6 mb-8 border border-[#D8D2FA] dark:border-white/10 shadow-sm flex flex-col md:flex-row items-center gap-4">
        <div className="relative w-full md:max-w-md">
          ...
          <input
            ...
            className="w-full pl-11 pr-4 py-2.5 rounded-xl bg-[#FCFBFF] dark:bg-[#1A1827] border border-[#D8D2FA] dark:border-white/15 focus:border-[#8B7FE8] focus:ring-4 focus:ring-[#D8D2FA]/50 text-[#1E1B2E] dark:text-white placeholder-[#6B6785] dark:placeholder-[#8E8A9F] text-sm font-medium outline-none transition-all min-h-[44px]"
          />
        </div>
        ...
        <div className="flex-1 w-full overflow-x-auto pb-2 md:pb-0 scrollbar-none flex gap-2">
          {allTags.map((tag) => (
            <button
              ...
              className={`px-3.5 py-1.5 rounded-full text-xs font-bold whitespace-nowrap transition-colors flex-shrink-0 min-h-[36px] ${
                activeTag === tag
                  ? "bg-[#8B7FE8] text-white shadow-soft-sm"
                  : "bg-[#F3F0FE] dark:bg-[#1E1933] text-[#6B6785] dark:text-[#A09CAE] hover:text-[#1E1B2E] dark:hover:text-white border border-[#EAE6FE] dark:border-white/10"
              }`}
            >
              {tag}
            </button>
