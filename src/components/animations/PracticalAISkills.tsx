...
export default function PracticalAISkills() {
  ...
  return (
    <section className="w-full bg-slate-50 dark:bg-[#0A061E] py-14 sm:py-20 lg:py-28 px-4 sm:px-8 lg:px-24 overflow-hidden relative transition-colors duration-300">
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
        
        {/* Left Side: Content */}
        <div className="flex flex-col z-10 text-center lg:text-left">
          <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-slate-900 dark:text-white tracking-tight leading-tight font-['Space_Grotesk'] mb-4 sm:mb-6 transition-colors duration-300">
            Upgrade your life <br />
            with practical <span className="text-[#8B7FE8]">AI skills</span>
          </h2>
          <p className="text-base sm:text-lg md:text-xl text-slate-600 dark:text-gray-300 leading-relaxed max-w-md mx-auto lg:mx-0 mb-6 sm:mb-8 transition-colors duration-300">
            Learn the tools. Build the skills. Get the certificate - in 28 days.
          </p>
          <div>
            <a href="#explore" className="inline-flex items-center justify-center bg-[#8B7FE8] hover:bg-[#786BD6] text-white font-semibold py-3.5 px-8 sm:px-10 rounded-xl transition-all duration-300 shadow-lg shadow-[#8B7FE8]/25 hover:shadow-xl hover:shadow-[#8B7FE8]/40 hover:scale-105 w-full sm:w-auto">
              Start Now →
            </a>
          </div>
        </div>
        ...
        <div className="relative h-[320px] sm:h-[400px] lg:h-[520px] overflow-hidden practical-ai-mask flex gap-3 sm:gap-5 lg:gap-6 justify-center lg:justify-end">
          
          {/* Column 1 - Moving Down */}
          <div className="flex flex-col gap-3 sm:gap-5 lg:gap-6 w-[90px] sm:w-[110px] lg:w-[140px] practical-ai-down">
            {col1.map((tool, index) => (
              <div key={`c1-${index}`} className="w-[90px] h-[90px] sm:w-[110px] sm:h-[110px] lg:w-[140px] lg:h-[140px] rounded-2xl sm:rounded-[1.75rem] overflow-hidden shadow-lg sm:shadow-xl shadow-black/20 flex-shrink-0 bg-white dark:bg-slate-900 flex items-center justify-center p-4 sm:p-6 border border-slate-100 dark:border-slate-800">
