...
export default function CourseLessonClient({ slug, lessonId }: CourseLessonClientProps) {
  ...
  const handleMarkComplete = async (skipRedirect = false, nextRoute?: string) => {
    if (!parentModule || !targetLesson) return;
    
    // Prevent double clicking / completing if already completed
    if (progressState?.completedLessonIds.includes(lessonId)) {
      if (!skipRedirect) router.push(`/courses/${slug}`);
      if (nextRoute) router.push(nextRoute);
      return;
    }

    // 1. Calculate XP locally
    let bonusXp = targetLesson.xpReward || 50;
    const allLessonsDone = parentModule.lessons.every(
      (l) => l.id === lessonId || progressState?.completedLessonIds.includes(l.id)
    );
    if (allLessonsDone && !parentModule.quiz && !progressState?.completedModuleIds.includes(parentModule.id)) {
      bonusXp += parentModule.xp;
    }

    // 2. Visual Effects (Confetti and Toast) - Side Cannons
    confetti({
      particleCount: 150,
      spread: 70,
      origin: { x: 0.1, y: 0.6 },
    ...
    toast.success(`Lesson Completed! +${bonusXp} XP gained!`, {
      icon: "🏆",
      style: {
        borderRadius: '10px',
        background: '#333',
    ...
    // 3. Make API request to update backend in background (don't await)
    try {
      const fetchHeaders: Record<string, string> = { "Content-Type": "application/json" };
      if (user?.email) fetchHeaders["X-User-Email"] = user.email;
      
      // Fire and forget fetch to not block navigation
      fetch("/api/activity/complete", {
        ...
        if (res.ok) {
          ...
          console.error("Backend activity error:", err);
        }
      }).catch(err => {
        console.error("Failed to update activity completion on backend:", err);
      });
    } catch (err) {
      console.error("Failed to initiate activity completion fetch:", err);
    }

    // 4. Update local state just for immediate UI feedback on this page
    setProgressState((prev) => {
      ...
      const reEvalAllLessonsDone = parentModule!.lessons.every(
        (l) => l.id === lessonId || updatedCompletedLessons.includes(l.id)
      );

      if (reEvalAllLessonsDone && !parentModule!.quiz && !updatedCompletedModules.includes(parentModule!.id)) {
    ...
    // 5. Navigate immediately without delay for seamless experience
    if (nextRoute) {
      router.push(nextRoute);
    } else if (!skipRedirect) {
      if (nextLesson) {
        router.push(`/courses/${slug}/lessons/${nextLesson.id}`);
      } else if (parentModule!.quiz) {
              ...
              <div className="flex items-center gap-3">
                ...
                {nextLesson && !nextLesson.locked ? (
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      handleMarkComplete(true, `/courses/${slug}/quiz/${parentModule!.id}`);
                    }}
                    className="px-5 py-3 rounded-2xl text-xs font-extrabold bg-[#8B7FE8] hover:bg-[#786BD6] text-white shadow-md transition-all flex items-center gap-2 cursor-pointer"
                  >
